...
exports.default = (0, _reduxActions.handleActions)((_handleActions = {}, _defineProperty(_handleActions, SPAWN, function (state) {
	...
	return _extends({}, state, {
		whacks: [],
		targets: Array.from({ length: random }).map(function () {
			var target = state.whacklist[Math.random() * state.whacklist.length | 0];
			return {
				id: target.id,
...
}), _defineProperty(_handleActions, WHACK, function (state, _ref) {
	var client = _ref.client,
	    _ref$payload = _ref.payload,
	    pos = _ref$payload.pos,
	    id = _ref$payload.id;

	var target = state.targets.find(function (t) {
		return t.id === id && t.pos === pos;
	});
	return _extends({}, state, {
		whacklist: state.whacklist.map(function (p) {
			return target && p.id === id ? _extends({}, p, { whacks: p.whacks + 1 }) : p;
		}),
		targets: state.targets.filter(function (t) {
			return t.pos !== pos && t.id !== id;
		}),
		whacks: target ? state.whacks.concat({ id: id, client: client, pos: pos }) : state.whacks
	});
